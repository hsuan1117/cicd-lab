...
> 請於繳交前將 GitHub Actions 成功執行的截圖貼於此處：
>
> - 截圖 1：Actions 列表頁，顯示綠色 ✓ 的 `CI (314551180)` workflow run。
> - 截圖 2：點進該 run 的 job 頁，可見 Typecheck / Prettier check / Run tests 等步驟皆綠勾。
> - 截圖 3：右上 "Vitest Tests" tab（dorny/test-reporter 產生），與下方 Job Summary 統計表。

`![成功-Actions列表](screenshots/success-list.png)`

`![成功-Job步驟](screenshots/success-job.png)`

`![成功-測試報告Tab](screenshots/success-report.png)`
...
> 請貼上 GitHub Actions 失敗執行的截圖：
>
> - 截圖 4：workflow run 顯示紅色 ✗ 的 summary 頁。
> - 截圖 5：`Run tests` step 展開，可看到 vitest 的紅色失敗訊息（expected 200, received 500）。
> - 截圖 6：dorny/test-reporter 產生的 "Vitest Tests" tab，顯示失敗 case 詳情。

`![失敗-Run總覽](screenshots/fail-summary.png)`

`![失敗-測試輸出](screenshots/fail-step.png)`

`![失敗-測試報告](screenshots/fail-report.png)`
